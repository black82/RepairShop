...
import {OtpoutDeviceComponent} from "../otpout-device/otpout-device.component";
...
import {RepairToPojoPaginator} from "../entity/RepairToPojoPaginator";

@Component({
  selector: 'app-repair-clouse-paginator',
  templateUrl: './repair-clouse-paginator.component.html',
  styleUrls: ['./repair-clouse-paginator.component.css']
})
export class RepairClousePaginatorComponent implements OnInit, OnDestroy {
  pageEvent: PageEvent;
  datasource: RepairToPojoPaginator [];
  pageIndex: number;
  pageSize: number;
  length: number;
  client_shouw: Client;
  ...
  constructor(
    // private outputClientComponent: OtpoutDeviceComponent,
    public cl: ClientStaticServiceService,
    private http: HttpClien, private fb: UntypedFormBuilder,
    ...
    // outputClientComponent.client_catch(null)
    this.hidden_form.form_open.emit(true);
    // setTimeout((()=>{this.hidden_form.form_open.emit(true);}),1000)
  ...
  ngOnInit(): void {
    ...
    //   this.client = client;
    // });
    // this.searchformComponent.hiddenFormAfterSubmitForm();
  }
  ...
  shows(element) {
    this.animation_wait.$anime_show.emit(true);
    this.paginator = false;
    this.id_repair = element?.repairId;
    const timeout = setTimeout(() => {
      // this.shouwClient = true;

      this.searchByIdRepair(element?.repairId)
      this.client_shouw = element;
      // this.client_catch(element)
      clearTimeout(timeout);
    }, 300);

  }

  searchByIdRepair(repairId) {
    if (!Number(repairId)) {
      this.alert_service.info(null, 'The value entered must be a Number.', false, false, null, null);
      return;
    }

    this.http.searchByRepairIdAndRepairArhiv(repairId).subscribe(
      client => {
        this.client = client;
        this.client_catch(client);
        this.animation_wait.$anime_show.emit(false);
      },
      () => {
      });
